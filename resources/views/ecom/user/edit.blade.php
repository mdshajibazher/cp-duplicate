...
<div class="col-lg-12">
	<div class="card">
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-8">
                    <p class="card-title float-right">Add New Customer</p>
                </div>
            </div>
        </div>
    <div class="card-body">
    ...
    <form action="{{route('ecomcustomer.update',$customer->id)}}" method="POST" style="border: 1px solid #ddd;padding: 20px;border-radius: 5px">
        @csrf
        @method('PUT')
        <div class="row">
        ...
        <div class="col-lg-6">
            ...
            <div class="form-group">
                ...
                <input type="text" id="email" placeholder="Enter Your Email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{old('email',$customer->custom_email)}}">
                @error('email')
                <small class="form-error">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select name="division" id="division" class="form-control @error('division') is-invalid @enderror" required>
                    ...
                    @foreach ($divisions as $item)
                        <option value="{{$item->id}}" {{ (old("division",$customer->division_id) == $item->id ? "selected": "") }}>{{$item->name}}</option>
                    @endforeach
